...
from toadhub.models import File, FileError
from utils.toad_logger import LogLevel


@pytest.fixture
def frog(mock_toad_logger):
    return mock_toad_logger("test_file", LogLevel.DEBUG)

@pytest.fixture
def mock_directory(mock_folder):
    return Directory(name="mock", parent=mock_folder.path)


def test_file_error(caplog, frog):
    with caplog.at_level(logging.INFO, logger=frog.name):
        error = FileError("Test error message.", frog)
        assert "Test error message." in caplog.text
        assert error.logger == frog


def test_file_init(mock_directory):
